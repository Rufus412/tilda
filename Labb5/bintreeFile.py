...
class Bintree:
    ...
    def find_path(self, item_str):
        if item_str in self:
            self._path(self.root, item_str)
        else:
            print("There is no path to: ", item_str)

    def _path(self, node, item_str) :
        if node != item_str:
            if node < item_str:
                if node.right:
                    print(node.value)
                    self._path(node.right, item_str)
            else:
                if node.left:
                    print(node.value)
                    self._path(node.left, item_str)
        else:
            print(item_str)
    ...
    def __search_and_transform(self, node: Node, result_list):
        if node.left:
            self.__search_and_transform(node.left, result_list)

        result_list.append(node)

        if node.right:
            self.__search_and_transform(node.right, result_list)

    def __transform_to_list(self):
        result_list = []
        self.__search_and_transform(self.root, result_list)
